    ...
    public partial class frmWebBrowser : Form
    {
        ...
        private void btnIr_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Uri uri = new Uri(url);
                Descargador downloader = new Descargador(uri);

                downloader.progreso += new Descargador.progresoCarga(this.ProgresoDescarga);
                downloader.descargaFinalizada += new Descargador.descargaCompleta(this.FinDescarga);

                Thread hilo = new Thread(downloader.IniciarDescarga);
                hilo.Start();

                this.archivos.leer(out historialLista);
                this.archivos.guardar(this.txtUrl.Text);
            }
            catch (Exception err)
            {
                this.rtxtHtmlCode.Text = err.Message;
            }
        }
